"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
const core_1 = require('@angular/core');
const router_1 = require('@angular/router');
const atividade_service_1 = require('../shared/atividade.service');
let AtividadeListComponent = class AtividadeListComponent {
    constructor(atividadeService, router) {
        this.atividadeService = atividadeService;
        this.router = router;
        this.loading = true;
    }
    gotoDetail(atividade) {
        this.router.navigate(['/atividades/', atividade.id]);
    }
    getAtividades() {
        this.atividadeService.getAtividades().then(atividades => {
            this.atividades = atividades;
            this.loading = false;
        });
    }
    ngOnInit() {
        this.getAtividades();
    }
    onSelect(atividade) {
        this.selectedAtividade = atividade;
    }
    add(titulo) {
        titulo = titulo.trim();
        if (!titulo) {
            return;
        }
        this.atividadeService.create(titulo)
            .then(atividade => {
            this.atividades.push(atividade);
            this.selectedAtividade = null;
        });
    }
    delete(atividade) {
        this.atividadeService
            .delete(atividade.id)
            .then(() => {
            this.atividades = this.atividades.filter(a => a !== atividade);
            if (this.selectedAtividade === atividade) {
                this.selectedAtividade = null;
            }
        });
    }
};
AtividadeListComponent = __decorate([
    core_1.Component({
        moduleId: module.id,
        selector: 'atividade-list',
        templateUrl: 'atividade-list.component.html'
    }), 
    __metadata('design:paramtypes', [atividade_service_1.AtividadeService, router_1.Router])
], AtividadeListComponent);
exports.AtividadeListComponent = AtividadeListComponent;
//# sourceMappingURL=atividade-list.component.js.map